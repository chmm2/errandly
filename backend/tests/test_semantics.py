...
def test_a_confident_genuine_reading_counts_for_the_group():
    assert _verdict(coherence=0.9, reads_as_genuine=True).exculpatory
...
def test_a_low_score_is_reported_but_carries_no_weight():
    ...
    assert not _verdict(coherence=0.05, reads_as_genuine=False).exculpatory
    details = semantics.verdict_details(_verdict(coherence=0.05, reads_as_genuine=False))
    # Reported to the admin, but flagged as carrying no exculpatory weight.
    assert details["semantic"]["coherence"] == 0.05
...
def test_a_high_score_with_a_negative_overall_reading_does_not_clear_anyone():
    ...
    assert not _verdict(coherence=0.95, reads_as_genuine=False).exculpatory
...
@async_test
async def test_no_api_key_means_no_opinion(monkeypatch):
    """The whole channel is optional. With no key the fraud system must behave
